...
async def test_create_tool_simple_name_with_extension(tmp_path):
    await verify_create_tool(library, "🛠️[create-file test.txt /]", "test.txt", tmp_path=tmp_path)


async def test_create_file_in_nonexistent_directory(tmp_path):
    await verify_create_tool(library, "🛠️[create-file src/utils/helper.py /]", "src/utils/helper.py",
                             tmp_path=tmp_path)
