...
async def test_logging_async_client_json_interaction(caplog):
    ...
    transport = httpx.MockTransport(
        lambda request: httpx.Response(
            200, json=response_data, headers={"Content-Type": "application/json"}
        )
    )

    with caplog.at_level(
        logging.DEBUG, logger="simple_agent.infrastructure.logging_http_client"
    ):
...
async def test_logging_async_client_data_interaction(caplog):
    transport = httpx.MockTransport(
        lambda request: httpx.Response(
            200, content="plain text response", headers={"Content-Type": "text/plain"}
        )
    )
    ...
    with caplog.at_level(
        ...
        async with LoggingAsyncClient(transport=transport) as client:
            await client.post("https://api.example.com/test", content="plain text body")
