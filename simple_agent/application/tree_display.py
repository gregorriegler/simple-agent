...
def generate_tree(root_path: Path = None, max_depth: int = 2) -> str:
    ...
    _build_tree(root_path, tree, parent=str(root_path), max_depth=max_depth, gitignore_patterns=gitignore_patterns)
...
def _build_tree(root_path: Path, tree: Tree, parent=None, max_depth=2, current_depth=0, gitignore_patterns=None):
    ...
    for item in items:
        ...
        if item.is_dir():
            _build_tree(item, tree, parent=node_id, max_depth=max_depth, current_depth=current_depth + 1, gitignore_patterns=gitignore_patterns)
